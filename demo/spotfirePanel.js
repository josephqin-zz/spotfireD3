  ...
  var height$2 = 1000;
	...
	var controlBar = function (_selection){
		...
		_selection.selectAll('g')
           ...
           .enter()
           .append('g')
           .attr('transform',(t,i)=>identity$8.translate(9,45+(tabHeight+2)*i))
           .each(function(d,i){
...
var groupsBar = function (_selection){
        ...
        var svg =_selection.selectAll('g')
        		  ...
        		  .append('g')
        		  .attr('id',(d,i)=>d.key)
        		  .attr('transform',(d,i)=>identity$8.translate(0,(metaData$1.length-i)*10+sum(metaData$1.filter((t,index)=>index>i).map((t)=>t.values.map((s)=>s.key).reduce((acc,d)=>acc>=d.length?acc:d.length,0)))*6));
...
var legendBar =function(_selection){
        ...
        var textsize = Math.floor(min(dataSet$2.map((d)=>d.width))/max(dataSet$2.map((d)=>d.name.length)));
		_selection.selectAll('g')
				  .data(dataSet$2)
				  .enter()
				  .append('g')
				  .attr('transform',(d,i)=>identity$8.translate(d.width*i,0))
				  .each(function(d,i){
				  	select(this).append('rect')
				  	              .attr('fill',d.color)
								  .attr('width',d.width)
...
var plotCanvas = function(_selection){
		...
		//render yAxis
		var chart = canvas.append('g').attr('id','plotWin').attr('transform',identity$8.translate(0,25));
		...
		chart.append("g").attr('id','mainParts')
			    ...
			    .enter()
				.append('path')
				.attr('transform',(d)=>identity$8.translate(d.x,d.y))
...
var linePlot = function(_selection){
		...
		var xScale = linear$2().range([0, width$2]);
		var yScale = linear$2().range([height$3, 0]);	    
		var vis = _selection.append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
		var color = ordinal().range(category20);
		var xAxis = axisBottom(xScale);
		var yAxis = axisLeft(yScale).ticks(5).tickFormat(format(".2s"));
		let Xmin = min(plotData.map((d)=>min(d.values.map((t)=>(t.x)))));
        let Xmax = max(plotData.map((d)=>max(d.values.map((t)=>(t.x)))));
        let Ymin = min(plotData.map((d)=>min(d.values.map((t)=>(t.y)))));
        let Ymax = max(plotData.map((d)=>max(d.values.map((t)=>(t.y)))));
...
'use strict'; 
  ...
  var dispatcher = dispatch('chromagraphUI','updateUI');

  //create page layout
  var leftBar = _selection.append('g').attr('id','leftBar');
  var legendBar$$1 = _selection.append('g').attr('id','legendBar').attr('transform',identity$8.translate(30,0));
  var mainCanvas = _selection.append('g').attr('id','mainCanvas').attr('transform',identity$8.translate(30,cellHeight));
  var groupBar = _selection.append('g').attr('id','groupBar').attr('transform',identity$8.translate(30,height$1+cellHeight));	
  var lineView = _selection.append('g').attr('id','lineView').attr('transform',identity$8.translate(width$1+40+30,cellHeight));
  ...
  {name:'std',type:'std',color:'#BBC7BA',selected:false}].map((b)=>{
    ...
    dispatcher.on('updateUI',function(newState){
       ...
       let color =  ordinal().range(category20).domain(colorGroup.map((d)=>d.key).filter((d,i,self)=>self.indexOf(d)===i));
       ...
       let xScale = band().range([0, width$1]).paddingInner(0.1).domain(sequence(xGroup.map((d)=>d.key).length));
      ...
      if(newState.chartType.indexOf('group')!==-1) xAxisData = metaData.filter((d,i,self)=>i<self.length-2);    
        ...
        let yScale = linear$2().range([height$1-25,0]);
        ...
        if(newState.chartType==='stackbar'){
          ...
          yScale.domain([0,max(chartData.values.map((t)=>t.sum))]);
          ...
          yAxis = axisRight(yScale).ticks(5).tickFormat(format(".2s")).tickSize(width$1);


        }else if(newState.chartType==='bar'){

          yScale.domain([0,max(chartData.values.map((t)=>typeof t.value.std==='undefined'?t.value.y:t.value.std+t.value.y))]);
          yAxis = axisRight(yScale).ticks(5).tickFormat(format(".2s")).tickSize(width$1);
          chartData.values.forEach((d,i)=>{
            d.x = newMap[d.key].x1;
         ...
         yScale = band().range([0,height$1-25]).domain(metaData[metaData.length-2].values.map((d)=>d.key).filter((d,i,self)=>self.indexOf(d)===i));
         yAxis = axisRight(yScale).tickSize(width$1);
         let arcFn = arc().innerRadius(0).outerRadius(min([yScale.bandwidth(),xScale.bandwidth()])/2);
         let pieFn = pie().sort((r)=>r.key).value((r)=>r.value.y);
        ...
        chartData.values.forEach((d,i)=>{
          ...
          d.color = color(xcMap[d.key]);
            ...
            //add line
            let linefn = line().x((d)=>d.x).y((d)=>d.y);
            let lines = nest().key((d)=>xcMap[d.key]).entries(chartData.values);
        ...
        .map((d)=>{
          ...
          item.values = d.values.map((t)=>{ return {
            key:t.key,
            x1: newMap[min(t.values)].x1,
            x2: newMap[max(t.values)].x2,
            values:[...t.values] }
    ...
    dispatcher.on('chromagraphUI',function(rawdata){
     lineView.selectAll('*').remove();
        	...
        	json('http://10.4.1.60/mtb/getData.php?type=mtb_chromat&peak_ids='+rawdata.value.peak_id.join(','),function(error,data){
        		if(error) return;
...
'use strict';
    ...
    utility.groupBy = function(groups){
        ...
        return nestfn.sortKeys(ascending);
    };
    //flaten NestData
    ...
    utility.groupFrame = function(groups){
        return groups.reduce((acc,d)=>{ 
                     ...
                     acc.push(sequence(acclength).map((i)=>d[i%d.length])); 
                     return acc; 
                    },[])
    };
...
var dispatcher = dispatch('updateUI','stopBy','selectOpt');
...
newMetaData.forEach((r)=>{
	   			if(r.name === root.name){
	   				...
	   				let maxv = max([0,...r.options.map((d)=>d.selected)]);
	...
	var roots = _selection.selectAll('g')
			  ...
			  .enter()
			  .append('g')
			  .attr('transform',(d,i)=>identity$8.translate(cellWidth*i,0))
	...
	roots
		.each(function(m,ri){
			select(this).append('g')
						   ...
						   .append('g')
						   .attr('id',(t,j)=>t.name)
						   .attr('transform',(t,j)=>identity$8.translate(0,cellHeight$1*(j+1)))
						   .attr('z-index',-1)
						   .each(function(t,j){
...
var groupsOpts=['reagent','stime', 'complex', 'dosage','cell_origin', 'cell_name', 'drug_sensitivity'];
	...
	var spotfirePanel = function(_selection){
        ...
        dispatcher.on('updateUI',function(newData){
            ...
            if(min(newData.map((d)=>d.options.filter((d)=>d.selected>-1).length))===0) return;
            // console.log(newData.map((d)=>d.options.filter((d)=>d.selected>-1).length))
            //get sample groups 
            let  rollupFn = (leave)=>mean(leave.map((d)=>d[yValue])),
                 stdFn = (leave)=>deviation(leave.map((d)=>d[yValue]));
        ...
        var plotUI = _selection.append('g').attr('id','plotUI').attr('transform',identity$8.translate(0,25));
        var menuBar = _selection.append('g').attr('id','menuBar').attr('transform',identity$8.translate(30,0));
        menuBar.call(selectionBar.bindData(menuBarData));
        var updateButton = _selection.append('g').attr('id','updateButton').attr('transform',identity$8.translate(800-150,2.5));
